...
function handleClientAttrs(el) {
  let clientClassEls = el.matches('[client-class]')
  ...
  let clientDisplayEls = el.matches('[client-display]')
...
async function scryDiff(comId) {
  ...
  const scryPath = scryBasePath + `/${uvurl}/${comId}` + comDomPath + '.json'
...
function buildComDomPath(comEl) {
  ...
  let kids = ''
  comEl.childNodes.forEach((kidEl) => {
    let kidNode = ''
    const kidComId = kidEl.getAttribute('mast')
    if (kidComId) {
      ...
      const t = 'mast'
      const k = kidEl.getAttribute('key')
      const h = '0'
      kidNode = `/${t}/${k}/${h}//`
    } else {
      kidNode = buildComDomPath(kidEl)
    }
    kids = kids + kidNode
  })
    ...
    switch (diffObj.p) {
      ...
      case 'c':
        ...
        while (targetNode.attributes.length > 0) {
          ...
          if (name === 'client-class' || name === 'client-display')
            clientAttrsChanged = true
        ...
        if (targetNode.tagName === 'CLIENT-STATE')
          handleClientStateElements(targetNode)
        break
      case 't':
...
function makeComSubDels(el) {
  ...
  els.forEach((cEl) => {
    let comAttr = cEl.getAttribute('mast')
    subAddActions.push(makeComUnsubscribe(comAttr))
  })
