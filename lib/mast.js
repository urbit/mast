...
addEventListener('DOMContentLoaded', () => {
    ...
    ses = document.documentElement.getAttribute('ses');
    sub = document.documentElement.getAttribute('sub');
    channelPath = `${window.location.origin}/~/channel/${ses}`;
    connectToShip();
...
async function handleChannelStream(event) {
    ...
    if (typeof msg.json === 'string') {
        handleNavigate(msg.json);
    } else {
        handleDiff(msg.json);
    };
};
function handleNavigate(pathStr) {
    sendActions([
        makeChannelDelete(),
        makePoke('kill', ses)
    ]);
    window.location.assign(pathStr);
};
...
function makePoke(tag, jsonData) {
    channelActId++;
    return {
        id: channelActId,
        action: 'poke',
